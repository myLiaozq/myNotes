...
Calendar.prototype = {
	...
	_draw: function(oCal, date) {
		...
		if(this.isSelect) {
			...
			oDiv.insertBefore(document.createTextNode("-"), this.selectMonth);
		...
		//文字年月
			...
			// oDt.innerHTML = date.year + "-" + newMonth + "\u6708";
			oDt.innerHTML = date.year + "-" + newMonth + "";
		}
		//日期
		oDd.innerHTML = "";
		oDd.appendChild(this._createDays(date.year, newMonth))
	},
	/**
	 * 生成日期
	...
	getDateInfo: function(sDate){
		var that = this,
			...
			aDateName = ["\u4eca\u5929", "\u660e\u5929", "\u540e\u5929"],
			sWeekName = "\u661f\u671f" + ["S", "M", "T", "W", "T", "F", "S"][oDate.getDay()],
			iDiff, p;
		return {
			week: sWeekName,
			holiday: function() {
