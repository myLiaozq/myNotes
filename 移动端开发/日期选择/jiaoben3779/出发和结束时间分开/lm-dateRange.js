    ...
    */
    ...
    if(this.device=='mobile'){
        $("#"+ this.inputId).bind('click', function() {
            ...
            for(var i=0; i<$('.m-ticket-tab>a').length; i++){
               
                if($($('.m-ticket-tab>a')[i]).hasClass('active')){
                    if($($('.m-ticket-tab>a')[i]).attr('data-way')=='oneWay'){
                        __method.mOpts.isSingleDay=true;
                        __method.mOpts.shortOpr=true;

                        __method.selectDate($('#' + __method.mOpts.startDateId).val());
        ...
        $('.m-ticket-tab>a').click(function(){
            var val = $(this).attr('data-way');
            // console.log(val);
            if(val=='oneWay'){
                $('#m-ticket-selectTime').val($('#' + __method.mOpts.startDateId).val());

               $('#startDate_data1').val($('#' + __method.mOpts.startDateId).val());
               $('#endDate_data1').val($('#' + __method.mOpts.startDateId).val());
               ...
               __method.mOpts.shortOpr=true;
                ...
                var str = $('#' + __method.mOpts.startDateId).val();

                var endTimeStr = new Date(new Date(Date.parse(str.replace(/-/g,  "/"))).getTime()+86400000*2);
                 
                var endTime = endTimeStr.getFullYear()+'-'+(endTimeStr.getMonth()+1)+'-'+(endTimeStr.getDate());

                $('#'+__method.inputId).val($('#' + __method.mOpts.startDateId).val()+' ~ '+endTime);

                $('#startDate_data1').val($('#' + __method.mOpts.startDateId).val());
                $('#endDate_data1').val(endTime);
        ...
        $('#'+__method.endId).attr("disabled",true); //初始化隐藏结束时间

        $("#"+ this.inputId+",#"+this.endId).bind('click', function() {
            __method.init();

            for(var i=0; i<$('.p-radioSelect>label').length; i++){
               
                if($($('.p-radioSelect>label')[i]).children('i').hasClass('active')){
                    if($($('.p-radioSelect>label')[i]).children('span').attr('data-way')=='oneWay'){
                        __method.mOpts.isSingleDay=true;
                        __method.mOpts.shortOpr=true;
        ...
        $('.p-radioSelect>label').click(function(){
            ...
            // console.log(val);
            if(val=='oneWay'){
                $('#'+__method.endId).attr("disabled",true);
                $('#'+__method.inputId).val($('#' + __method.mOpts.startDateId).val());

               $('#startDate_data2').val($('#' + __method.mOpts.startDateId).val());
               $('#endDate_data2').val($('#' + __method.mOpts.startDateId).val());

               __method.mOpts.isSingleDay=true;
               __method.mOpts.shortOpr=true;

            }else if(val=='roundTrip'){
                $('#'+__method.endId).attr("disabled",false);
                ...
                $('#'+__method.inputId).val($('#' + __method.mOpts.startDateId).val());
                $('#'+__method.endId).val(endTime);

                $('#startDate_data2').val($('#' + __method.mOpts.startDateId).val());
                $('#endDate_data2').val(endTime);
