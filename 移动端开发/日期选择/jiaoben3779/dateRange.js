...
function pickerDateRange(inputId, options) {
    ...
    $('#' + this.inputId).bind('click', function() {
        ...
        for(var i=0; i<$('#main>input').length; i++){
            ...
            if($($('#main>input')[i]).is(':checked')){
                if($($('#main>input')[i]).val()=='oneWay'){
    ...
    $('#main>input').click(function(){
        var val = $(this).val();
        if(val=='oneWay'){
            $('#date1').html($('#' + __method.mOpts.startDateId).val());
            ...
            $('#date1').html($('#' + __method.mOpts.startDateId).val()+' ~ '+endTime);
...
pickerDateRange.prototype.show = function(isCompare, __method) {
    ...
    $('#' + __method.dateRangeDiv).css('display', isCompare ? 'none' : ''); //是否显示日期左下角 日期显示框
    ...
    var left = pos.left;
    $("#" + this.calendarId).css('display', 'block'); //显示日期插件
    if (true == this.mOpts.singleCompare || true == this.mOpts.isSingleDay) {
        $('#' + this.endDateId).css('display', 'none'); //左下角 结束时间隐藏
        $('#' + this.endCompareDateId).css('display','none');
        $('#' + this.mOpts.joinLineId).css('display', 'none'); //间隔线隐藏
        $('.' + this.mOpts.joinLineId).css('display', 'none');
    }else{
         $('#' + this.mOpts.joinLineId).css('display', 'inline-block'); //间隔线显示
